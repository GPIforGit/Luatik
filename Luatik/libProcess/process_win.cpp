...
Process::id_type Process::open(const string_type &command, const string_type &path, const environment_type *environment) noexcept {
  ...
  BOOL bSuccess = CreateProcessW(nullptr, process_command.empty() ? nullptr : &process_command[0], nullptr, nullptr,
    ...
    stdout_thread = std::thread([this]() {
      ...
      for(;;) {
       ...
       
        {
            ...
            stdout_output += std::string(buffer.get(), static_cast<size_t>(n));
        }
    ...
    stderr_thread = std::thread([this]() {
      ...
      for(;;) {
...

        {
            ...
            stderr_output += std::string(buffer.get(), static_cast<size_t>(n));
        }
